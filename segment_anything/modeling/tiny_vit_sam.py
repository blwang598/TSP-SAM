...
class TinyViT(nn.Module):
    ...
    def forward_features(self, x):
        ...
        output_embeddings = []
        multi_scale_layer_feature_block_length = int(len(self.layers) - 1)
        for i in range(multi_scale_layer_feature_block_length):
            layer = self.layers[i]
            x = layer(x)
            B, L, C = x.size()
            H, W = int(math.sqrt(L)), int(math.sqrt(L))
            output_embeddings.append(x.view(B, H, W, C).permute(0, 3, 1, 2))
        x = self.layers[-1](x)
        B, _, C = x.size()
        x = x.view(B, 64, 64, C)
        x = x.permute(0, 3, 1, 2)
        x = self.neck(x)
        return x, output_embeddings

    def forward(self, x):
        x, output_embeddings = self.forward_features(x)
        return x, output_embeddings
